...
def main():
    ...
    # MaxGPT-3 optimization: torch.compile JITs the model into fused CUDA kernels.
    # First iteration is slow (compilation, ~30-90 sec) but every subsequent step is
    # ~20-40% faster. For a multi-million-step run, this saves 1-2 days of training.
    # Falls back to eager mode if compile errors (some PyTorch+Blackwell combos hit
    # issues with custom sdpa_kernel contexts).
    ...
    # fused=True uses the fused CUDA kernel for the AdamW update step. Small
    # per-step speedup, free win since it's a single flag.
