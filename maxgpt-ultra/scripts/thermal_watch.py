...
sessions and every train/burn/bench process, then keeps logging so the cool-down is recorded) when:
  - any GPU reaches --max-temp (default: 4 C under the driver's own slowdown temperature),
  - the idle "thermometer" GPUs rise --idle-rise C above their baseline (room heating up),
  - any GPU reports a hardware/thermal slowdown, or a CPU sensor passes --max-cpu.
...
def read_gpus(smi: str) -> list[dict]:
    ...
    for line in out.strip().splitlines():
        ...
        rows.append({"gpu": int(v[0]), "temp": num(v[1]), "power": num(v[2]), "util": num(v[3]),
                     "fan": num(v[4]), "sm": num(v[5]),
                     "throttle": any(x.lower().startswith("active") for x in v[6:9])})
    return rows


def slowdown_temp(smi: str) -> float | None:
    try:
        out = subprocess.run([smi, "-q", "-d", "TEMPERATURE"], capture_output=True, text=True, timeout=10).stdout
        for line in out.splitlines():
            if "Slowdown Temp" in line and "C" in line:
...
def main() -> None:
    ...
    ap.add_argument("--max-temp", type=float, default=None, help="trip temp for any GPU (default: slowdown temp - 4)")
    ...
    sd = slowdown_temp(args.smi)
    max_temp = args.max_temp if args.max_temp is not None else ((sd - 4.0) if sd else 85.0)
    ...
    log(f"watch start: max_temp={max_temp:.0f}C (driver slowdown {sd}), idle_rise={args.idle_rise}C on GPUs {therm}, "
    ...
    while True:
        ...
        with open(args.log, "a", newline="", encoding="utf-8") as f:
            ...
            if new:
                ...
                new = False
            for g in gpus:
                w.writerow([ts, g["gpu"], g["temp"], g["power"], g["util"], g["fan"], g["sm"], int(g["throttle"]), ct])
        if base_val is None:
            for g in gpus:
        ...
        print(f"{ts} loaded={len(loaded)} hottest=gpu{hottest['gpu'] if hottest else '?'} "
              ...
              f"total={total_w:.0f}W cpu={ct:.0f}C", flush=True)
        if not tripped:
            reason = None
            if hottest and hottest["temp"] >= max_temp:
                reason = f"gpu{hottest['gpu']} at {hottest['temp']:.0f}C >= {max_temp:.0f}C"
            elif any(g["throttle"] for g in gpus):
                reason = "GPU reports hardware/thermal slowdown: " + ",".join(str(g["gpu"]) for g in gpus if g["throttle"])
